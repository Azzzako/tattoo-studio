...
export function AvatarUpload({ initialUrl }: { initialUrl: string | null }) {
  const [state, formAction, pending] = useActionState<
    ...
    FormData
  >(uploadAvatarProposal, undefined);
  ...
  if (state?.ok) {
    return (
      <div className="space-y-2">
        ...
        <SuccessBanner
          title="Avatar enviado al admin"
          description="Tu nueva foto queda pendiente de aprobacion. Tu avatar publico sigue siendo el actual hasta que el admin apruebe."
        />
      </div>
    );
  }
    ...
    <form ref={formRef} action={formAction} className="space-y-3">
      ...
      <div className="flex flex-col items-start gap-3">
        ...
        <p className="text-muted-foreground text-xs">
          JPG/PNG/WebP · max. 5 MB · requiere aprobacion del admin
        </p>
        {state?.ok === false && (
